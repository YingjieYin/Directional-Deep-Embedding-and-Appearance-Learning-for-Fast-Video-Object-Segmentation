...
class SynthesisVD(torch.utils.data.Dataset):
...
#        assert samplelen > 1, "samplelen must be at least 2"
    ...
    def _init_data(self):
        ...
        """
        print("-- SynthesisVD dataset initialization started.")
        framework_path = os.path.join(os.path.dirname(__file__), '..')
        cache_path = os.path.join(framework_path, 'cache', 'SynthesisVD_visible_objects_100px_threshold.json')
        ...
        if os.path.exists(cache_path):
            ...
            seqnames = os.listdir(os.path.join(self._root_path, 'JPEGImages'))
            ...
            for seqname in seqnames:
                ...
                self._visible_objects[seqname] = OrderedDict(
                    (self._frame_name_to_idx(os.path.basename(path)),
                     get_anno_ids(path, dataset_utils.LabelToLongTensor(), 300))
    ...
    def get_anno_frame_names(self, seqname):
        return os.listdir(os.path.join(self._root_path, "Annotations",  seqname))
    ...
    def get_image_frame_names(self, seqname):
        return os.listdir(os.path.join(self._root_path, "JPEGImages", seqname))
    ...
    def _full_image_path(self, seqname, image):
        ...
        return os.path.join(self._root_path, 'JPEGImages', seqname, image)

    def _full_anno_path(self, seqname, anno):
        if isinstance(anno, int):
            anno = self._frame_idx_to_anno_fname(anno)
        return os.path.join(self._root_path, 'Annotations', seqname, anno)
