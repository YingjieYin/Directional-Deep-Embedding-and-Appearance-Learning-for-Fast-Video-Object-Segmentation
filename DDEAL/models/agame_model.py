...
class GaussiansAgame(nn.Module):
    ...
    def get_init_state(self, ref_feats, ref_seg):
        ...
        if self.residual:
            ...
            residual = F.relu(ref_seg[:, :, :, :] - test_classification[:, :, :, :]).split(1, dim=1)
...
class GlobalMatchingcos(nn.Module):
    def __init__(self, nchannels_in, nchannels_lda):
        super().__init__()
        self.conv_in = nn.Conv2d(nchannels_in, nchannels_lda, 1)
        nn.init.kaiming_uniform_(self.conv_in.weight)
    def get_global_kernels(self, feats, given_seg):

        conv_feats_norm = self.get_conv_feats_norm(feats)
        # feature normalization

        # count up the background and object norm vectors
        gmcos_kernel_lst = []
        # for i in range(B):
        #     bg_index = (given_seg[i,0]>given_seg[i,1]).nonzero()
        #     bg_size = bg_index.size()[0]
        #     ob_index = (given_seg[i,0]<=given_seg[i,1]).nonzero()
        #     ob_size =
        gmcos_kernel = conv_feats_norm.reshape(conv_feats_norm.size(0),
                                                   conv_feats_norm.size(1),conv_feats_norm.size(2)*conv_feats_norm.size(3))
        gmcos_kernel = gmcos_kernel.permute([0,2,1])
        given_seg_bg = given_seg[:,0,:,:]
        given_seg_bg = given_seg_bg.reshape(given_seg_bg.size(0),
                                            given_seg_bg.size(1)*given_seg_bg.size(2),1)
        given_seg_bg = given_seg_bg.repeat(1,1,512)

        given_seg_ob = given_seg[:,1,:,:]
        given_seg_ob = given_seg_ob.reshape(given_seg_ob.size(0),
                                            given_seg_ob.size(1)*given_seg_ob.size(2),1)
        given_seg_ob = given_seg_ob.repeat(1,1,512)

        gmcos_kernel_bg = gmcos_kernel * given_seg_bg
        gmcos_kernel_ob = gmcos_kernel * given_seg_ob

        return [gmcos_kernel_bg,gmcos_kernel_ob]
    def get_conv_feats_norm(self, feats):
        B, C, H, W = feats.size()
        N = H * W
        conv_feats = self.conv_in(feats)
        conv_feats_norm = conv_feats / (1 / (N * N) + conv_feats.norm(dim=1, keepdim=True))
        return conv_feats_norm
    def forward(self, feats,state):
        conv_feats_norm = self.get_conv_feats_norm(feats)
        gmcos_kernel_bg, gmcos_kernel_ob =state['gm_kernel']
        bg_ob_corr_lst = []

        B = gmcos_kernel_bg.size(0)
        for i in  range(B):
            temp_bg=F.conv2d(conv_feats_norm[i:i+1],gmcos_kernel_bg[i].unsqueeze(2).unsqueeze(3))
            temp_bg,_=temp_bg.max(1,keepdim=True)
            temp_ob=F.conv2d(conv_feats_norm[i:i+1],gmcos_kernel_ob[i].unsqueeze(2).unsqueeze(3))
            temp_ob, _ = temp_ob.max(1, keepdim=True)
            bg_ob_corr_lst.append(torch.cat([temp_bg,temp_ob],1))

        bg_ob_scores = torch.cat(bg_ob_corr_lst,0)
        return bg_ob_scores
class vMFAgame(nn.Module):
    def __init__(self, nchannels_in, nchannels_lda, lr, cov_reg_init=30, residual=True, covest=True):
        super().__init__()
        self.lr = lr
        self.residual = residual
        self.covest = covest
        self.conv_in = nn.Conv2d(nchannels_in, nchannels_lda, 1)
        #self.cov_reg = nn.Parameter(cov_reg_init * torch.ones(4, 1, nchannels_lda, 1, 1))
        self.k_reg = nn.Parameter(cov_reg_init * torch.ones(2))
        nn.init.kaiming_uniform_(self.conv_in.weight)
    def get_init_state(self, ref_feats, ref_seg):
        B, C, H, W = ref_feats['s16'].size()
        N = H * W
        ...
        conv_ref_feats = self.conv_in(ref_feats['s16'])
        #feature normalization
        conv_ref_feats_norm = conv_ref_feats/(1/(N*N)+conv_ref_feats.norm(dim=1,keepdim=True))
        if np.isnan(conv_ref_feats.norm(dim=1, keepdim=True).detach().cpu().numpy()).any():
            iiiii=1
        #calculate means
        means = [F.adaptive_avg_pool2d(conv_ref_feats_norm * ref_seg[:,i:i+1,:,:], 1)/
                 (1/(N*N)+F.adaptive_avg_pool2d(conv_ref_feats_norm * ref_seg[:,i:i+1,:,:], 1).norm(dim=1,keepdim=True))
                 for i in range(K1)]
        for i in range(K1):
          if np.isnan(means[i].norm(dim=1, keepdim=True).detach().cpu().numpy()).any():
            iiiii=1
        k_vmf = F.softplus(self.k_reg)
        # means = [F.adaptive_avg_pool2d(conv_ref_feats * ref_seg[:,i:i+1,:,:], 1)
        #          / (1/N + F.adaptive_avg_pool2d(ref_seg[:,i:i+1,:,:], 1))
        #          for i in range(K1)]
        #distances = [(conv_ref_feats - means[i]) for i in range(K1)]
        ...
        if self.residual:
            test_classification = F.softmax(self.forward(ref_feats, {'appmod': (means, k_vmf)})[0], dim=-3)
            #residual = F.relu(test_classification[:,:,:,:] - ref_seg[:,:,:,:]).split(1, dim=1)
            residual = F.relu(ref_seg[:, :, :, :] - test_classification[:, :, :, :]).split(1, dim=1)
            # means += [(F.adaptive_avg_pool2d(conv_ref_feats * residual[idx], 1)
            #            / (1/N + F.adaptive_avg_pool2d(residual[idx], 1)))
            ...
            # distances += [(conv_ref_feats - means[i+2]) for i in range(K1)]
            means += [F.adaptive_avg_pool2d(conv_ref_feats * residual[idx], 1) /
                     (1/(N*N)+F.adaptive_avg_pool2d(conv_ref_feats * residual[idx], 1).norm(dim=1, keepdim=True))
                     for idx in range(K1)]
            for i in range(K1):
                if np.isnan(means[i].norm(dim=1, keepdim=True).detach().cpu().numpy()).any():
                    iiiii = 1
    ...
    def update(self, feats, seg, state):
        ...
        # covariances = [(1 - self.lr) * o + self.lr * n for o,n in zip(old_covariances, new_covariances)]
        #return means, covariances
        old_means, old_k_vmf = state['appmod']
        new_means, new_k_vmf = self.get_init_state(feats, seg)
        means = [(1 - self.lr) * o + self.lr * n for o,n in zip(old_means, new_means)]
        k_vmf = [(1 - self.lr) * o + self.lr * n for o,n in zip(old_k_vmf, new_k_vmf)]
        return means, k_vmf
    def forward(self, feats, state):
        means, k_vmf = state['appmod']
        nclasses = len(means)
        conv_feats = self.conv_in(feats['s16'])
        #features normalization
        conv_ref_feats_norm = conv_feats / conv_feats.norm(dim=1, keepdim=True)
        if np.isnan(conv_feats.norm(dim=1, keepdim=True).detach().cpu().numpy()).any():
            iiiii=1
        corr_logit = []
        for k in range(nclasses):
            corr_logit_temp = [F.conv2d(k_vmf[k%2]*conv_ref_feats_norm[i:i+1],means[k][i:i+1])
            for i in range(conv_ref_feats_norm.size(0))]
            corr_logit+=[torch.cat(corr_logit_temp,0)]
        score_tensor = torch.cat(corr_logit,dim=-3)

        if np.isnan(score_tensor.detach().cpu().numpy()).any():
            iiiii = 1
        # distances = [(conv_feats - means[i]) for i in range(nclasses)]
        # scores = [- 1/2 * (torch.log(covariances[i])
        #                    + (distances[i] * distances[i]) / covariances[i]).sum(dim=-3,keepdim=True)
        ...
        # scores_tensor = torch.cat(scores, dim=-3)
        #return scores_tensor, (means, covariances)
        return score_tensor, (means, k_vmf)
...
class FusionAgame(nn.Module): # Feed Forward Stack
    ...
    def forward(self, app_feats, dyn_feats,gmcos_feat, state):
        h = torch.cat([gmcos_feat,app_feats, dyn_feats], dim=-3)
        h = self.layers(h)
...
APPMODS = {'vMFAgame': vMFAgame, 'GaussiansAgameHack': GaussiansAgameHack}
...
GMCOSMODS = {'GlobalMatchingcos': GlobalMatchingcos}
...
class TrackSeg(nn.Module):
    def __init__(self, backbone_cfg, appmod_cfg, dynmod_cfg, gmcos_cfg,fusmod_cfg, segmod_cfg):
        super().__init__()
        ...
        self.gmcos    = GMCOSMODS[gmcos_cfg[0]](*gmcos_cfg[1])
        self.fusmod   = FUSMODS[fusmod_cfg[0]](*fusmod_cfg[1])
        self.segmod   = SEGMODS[segmod_cfg[0]](*segmod_cfg[1])
    ####new added
    def get_first_frame_ob_bg_corr_kernel(self,feats,given_seg,state):

        state['gm_kernel'] = self.gmcos.get_global_kernels(feats['s16'],given_seg)
    ...
    def forward(self, feats, state):
        ...
        gmcos_output= self.gmcos(feats['s16'],state)
        fused_output, coarse_segscore = self.fusmod(appmod_output, dynmod_output, gmcos_output, state)
...
class AGAME(nn.Module):
    def __init__(self,
                 ...
                 appearance=('vMFAgame', (2048, 512, .1, 30)),
                 dynamic=('RGMPLike', ([('ConvRelu', (2*2048+2*2,512,1,1,0)),
                                        ('DilationpyramidRelu',(512,512,3,1,(1,3,6),(1,3,6))),
                                        ('ConvRelu',(1536,512,3,1,1))],)),
                 gmcos = ('GlobalMatchingcos', (2048, 512)),
                 fusion=('FusionAgame',
                         ([('ConvRelu',(516+2,512,3,1,1)), ('ConvRelu',(512,128,3,1,1))],
                          [('Conv', (128, 2, 3, 1, 1))])),
        ...
        self.trackseg = TrackSeg(backbone, appearance, dynamic, gmcos,fusion, segmod)
    ...
    def forward(self, x, given_labels=None, state=None):
        ...
        if state is None:
            ...
            for obj_idx in object_ids:
                ...
                state[obj_idx],feats_first_frame = self.trackseg.get_init_state(video_frames[0], given_seg)
                state[obj_idx]=self.trackseg.get_first_frame_ob_bg_corr_kernel(feats_first_frame, given_seg, state[obj_idx])
            ...
            if isinstance(given_labels, (list, tuple)) and given_labels[i] is not None and i != 0:
                ...
                for obj_idx in new_object_ids:
                    ...
                    if state.get(obj_idx) is None:
                        ...
                        state[obj_idx],feats_first_frame = self.trackseg.get_init_state(video_frames[i], given_seg)
                        state[obj_idx] = self.trackseg.get_first_frame_ob_bg_corr_kernel(feats_first_frame, given_seg,
                                                                                         state[obj_idx])
                        object_visibility[obj_idx] = i
                    else:
                        raise NotImplementedError("Received a given (ground-truth) segmentation for an object idx that is already initialized. This could happen in the future, but should not happen with standard VOS datasets. Existing ids are {} and new ids are {}".format(state.keys(), new_object_ids))
