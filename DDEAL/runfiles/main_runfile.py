...
from dataset_loaders import IMAGENET_MEAN, IMAGENET_STD,SynthesisVD, DAVIS17V2, YTVOSV2, LabelToLongTensor
import evaluation
import models
import trainers
import utils
...
def train_alpha(model):
    ...
    train_set = torch.utils.data.ConcatDataset([
         SynthesisVD(config['SynthesisVD_path'], '2017', 'train', image_read, label_read, train_transform, nframes,
   ...
   # print("Sets initiated with {} (train), {} (val), and {} (test) samples.".format(len(train_set), len(val_set)))
    ...
    trainer = trainers.VOSTrainer(
        ...
        checkpoint_interval=2, print_interval=25, debug=False)
...
def train_beta(model):
    ...
    sampler = torch.utils.data.WeightedRandomSampler(len(train_set.datasets[0]) * [0.5*1 / len(train_set.datasets[0])] + len(train_set.datasets[1]) * [1 / len(train_set.datasets[1])]+len(train_set.datasets[2]) * [1 / len(train_set.datasets[2])], 60*len(train_set.datasets[1]), replacement=False)
...
def test_model(model, debug_sequences_dict=None, save_predictions=False, prediction_path=None):
    ...
    datasets = {
         ...
         'YTVOS_jval': YTVOSV2(config['ytvos_path'], 'train', 'val_joakim', 'JPEGImages', image_read, label_read,
                               None, nframes),
         'YTVOS_val': YTVOSV2(config['ytvos2018_path'], 'valid', None, 'JPEGImages', image_read, label_read,
                             None, nframes)
    }
